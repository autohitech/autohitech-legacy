...
/* 업로드 시작 */
$upload = file_upload($_FILES['bn_file'],$upload_dir);

if(!$bn_file_chk){
	if($upload[0][file_name]){
		$sql_file .= "bn_file_name = '{$upload[0][file_name]}',";
		$sql_file .= "bn_file_source = '{$upload[0][real_name]}',";
		$sql_file .= "bn_file_type = '{$upload[0][image][2]}',";
	}
}else{
	$rs = sql_fetch("select bn_file_name from $g4[banner_table] where bn_id = $bn_id");
	@unlink($upload_dir.$rs['bn_file_name']);
		$sql_file .= "bn_file_name = '',";
		$sql_file .= "bn_file_source = '',";
		$sql_file .= "bn_file_type = '',";
}


$sql_common = " bn_location = '$bn_location',
						bn_url = '$bn_url',
						bn_target = '$bn_target',
						bn_openchk = '$bn_openchk',
						bn_start_date = '$bn_start_date',
						bn_end_date = '$bn_end_date',
...
if ($w == 'u'){
	$sql = " update $g4[banner_table] set 
				$sql_common 
			  where bn_id = '$bn_id' ";
	sql_query($sql);
}else{
    $sql = " insert $g4[banner_table] set 
				$sql_common ";
    sql_query($sql);
    $bn_id = mysql_insert_id();
}

goto_url("./banner_form.php?$qstr&w=u&bn_id=$bn_id");
?>
