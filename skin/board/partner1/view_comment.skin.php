...
<? } 
    if (!$member[mb_id])  
    {
$login_url = "./login.php?wr_id=$wr_id{$qstr}&url=".urlencode("./board.php?bo_table=$bo_table&wr_id=$wr_id");
?>
 <script language='javascript'>
      function form_check(sun) {
         if(confirm('로그인 하셔야 글을 입력하실 수 있습니다.\n\n 로그인하시겠습니까? ')==true) {
...
<tr>
    <td width=95%>
        <textarea id="wr_content" name="wr_content" rows=8 itemname="내용" required onclick='return form_check(this)' style='width:100%; word-break:break-all;' class=tx></textarea>
    </td>
    <td width=85 align=center>
        <div><input type="image" src="<?=$board_skin_path?>/img/co_btn_write.gif" border=0 accesskey='s'></div>
    </td>
